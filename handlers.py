    ...
    # ШАХТА
        ...
        if not user.get("is_premium"):
            ...
            if "error" not in mine_info:
                await callback.message.edit_text(
                    f"⛏️ **Шахта**\n\n"
                    f"Уровень: {mine_info['level']}\n"
                    f"Добыча: {mine_info['daily_output']:.2f} PAC/день\n"
                    f"Накоплено: {mine_info['accumulated']} PAC",
                    reply_markup=get_back_keyboard(),
                    parse_mode="Markdown"
                )
            else:
                await callback.message.edit_text(mine_info["error"], reply_markup=get_back_keyboard())
