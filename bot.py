...
# ЮKassa данные (ЗАМЕНИТЕ НА СВОИ!)
...
def get_main_menu():
    keyboard = [
        ...
        [InlineKeyboardButton("💳 Пополнить (ЮKassa)", callback_data='recharge_yookassa')],
...
async def recharge_yookassa(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = str(update.effective_user.id)
    balances = load_json(BALANCE_FILE)
    balance = balances.get(user_id, 0)
    
    keyboard = [
        [InlineKeyboardButton("💳 50 ₽ → +500 PAK", callback_data='pay_50')],
        [InlineKeyboardButton("💳 100 ₽ → +1000 PAK", callback_data='pay_100')],
        [InlineKeyboardButton("💳 250 ₽ → +2500 PAK", callback_data='pay_250')],
        [InlineKeyboardButton("💳 500 ₽ → +5000 PAK", callback_data='pay_500')],
        [InlineKeyboardButton("💳 1000 ₽ → +10000 PAK", callback_data='pay_1000')],
        [InlineKeyboardButton("🔙 Назад", callback_data='menu')]
    ]
    ...
    await update.message.reply_text(
        ...
        f"• 500 ₽ → +5000 PAK\n"
        f"• 1000 ₽ → +10000 PAK\n\n"
        f"🔒 Безопасная оплата банковской картой",
        reply_markup=InlineKeyboardMarkup(keyboard),
        parse_mode='Markdown'
    )

async def create_yookassa_payment(update, amount_rub, user_id, username):
    ...
    payment_data = {
        ...
        "description": f"Пополнение баланса: +{amount_pak} PAK",
        "metadata": {
            "user_id": user_id,
            "amount_rub": amount_rub,
            "amount_pak": amount_pak,
    ...
    if response.status_code == 200:
        ...
        return payment_info["confirmation"]["confirmation_url"], payment_id
    else:
        return None, None
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif query.data == 'recharge_yookassa':
        keyboard = [
            [InlineKeyboardButton("💳 50 ₽ → +500 PAK", callback_data='pay_50')],
            [InlineKeyboardButton("💳 100 ₽ → +1000 PAK", callback_data='pay_100')],
            [InlineKeyboardButton("💳 250 ₽ → +2500 PAK", callback_data='pay_250')],
    ...
    elif query.data in ['pay_50', 'pay_100', 'pay_250', 'pay_500', 'pay_1000']:
        amounts = {'pay_50': 50, 'pay_100': 100, 'pay_250': 250, 'pay_500': 500, 'pay_1000': 1000}
        rub_amount = amounts[query.data]
        pak_amount = rub_amount * RUB_TO_PAK
        
        # Демо-режим (без реальной оплаты)
        balances[user_id] = balance + pak_amount
        save_json(BALANCE_FILE, balances)
        
        await query.edit_message_text(
            f"✅ **Пополнено!**\n\n"
            f"💳 {rub_amount} ₽ → +{pak_amount} PAK\n"
            f"💰 Новый баланс: {balances[user_id]:.0f} PAK\n\n"
            f"💡 Реальная оплата через ЮKassa будет подключена позже",
            reply_markup=get_main_menu(),
            parse_mode='Markdown'
        )
...
def main():
    ...
    print("✅ Пополнение через ЮKassa (демо-режим)")
    print("✅ 1 ₽ = 10 PAK")
...
Как подключить реальную ЮKassa:
...
1. Вместо демо-пополнения вставьте реальный код:
...
```python
    ...
    payment_url, payment_id = await create_yookassa_payment(
        update, rub_amount, user_id, update.effective_user.username
    )
...
1 Кубик чет 100 x1.8
2 Множитель множитель 100 5 x0.25-x1.5
3 Рулетка рулетка 100 7 x2-x35
4 Слоты слоты 100 x2-x50
5 КНБ кнб 100 камень x2
6 Блэкджек блэкджек 100 x2
7 Кости кости 100 7 x2-x5
