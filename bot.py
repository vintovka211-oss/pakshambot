...
ADMIN_ID = "8493522297"
REFERRAL_PERCENT = 10
DAILY_BONUS = 500  # 500 PAK (50 рублей)
STARS_TO_PAK = 20  # 1 Star = 20 PAK
RUB_TO_PAK = 10    # 1 рубль = 10 PAK

# ========== ГЛАВНОЕ МЕНЮ ==========
def get_main_menu():
    keyboard = [
        [InlineKeyboardButton("💰 Баланс", callback_data='balance')],
        [InlineKeyboardButton("⭐ Пополнить (Stars)", callback_data='recharge')],
        [InlineKeyboardButton("💳 Пополнить (Рубли)", callback_data='recharge_rub')],
...
def get_games_menu():
    keyboard = [
        ...
        [InlineKeyboardButton("🃏 Блэкджек (21)", callback_data='blackjack_game')],
        [InlineKeyboardButton("🎲 Кости (Сума)", callback_data='dice_sum_game')],
...
async def recharge_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    balance = balances.get(user_id, 0)
    keyboard = [
        [InlineKeyboardButton("⭐ 5 Stars → +100 PAK", callback_data='pay_5')],
        [InlineKeyboardButton("⭐ 10 Stars → +200 PAK", callback_data='pay_10')],
        [InlineKeyboardButton("⭐ 25 Stars → +500 PAK", callback_data='pay_25')],
        [InlineKeyboardButton("⭐ 50 Stars → +1000 PAK", callback_data='pay_50')],
        [InlineKeyboardButton("⭐ 100 Stars → +2000 PAK", callback_data='pay_100')],
        [InlineKeyboardButton("🔙 Назад", callback_data='menu')]
    ]
    await update.message.reply_text(
        f"💰 Баланс: {balance:.0f} PAK ({balance/RUB_TO_PAK:.2f} ₽)\n\n"
        f"⭐ **Пополнение за Stars**\n"
        f"💎 1 Star = 20 PAK\n\n"
        f"Выбери сумму:",
...
# ========== ПОПОЛНЕНИЕ ЗА РУБЛИ ==========
async def recharge_rub_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = str(update.effective_user.id)
    balances = load_json(BALANCE_FILE)
    balance = balances.get(user_id, 0)
    
    keyboard = [
        [InlineKeyboardButton("💳 50 ₽ → +500 PAK", callback_data='rub_50')],
        [InlineKeyboardButton("💳 100 ₽ → +1000 PAK", callback_data='rub_100')],
        [InlineKeyboardButton("💳 250 ₽ → +2500 PAK", callback_data='rub_250')],
        [InlineKeyboardButton("💳 500 ₽ → +5000 PAK", callback_data='rub_500')],
        [InlineKeyboardButton("💳 1000 ₽ → +10000 PAK", callback_data='rub_1000')],
        [InlineKeyboardButton("🔙 Назад", callback_data='menu')]
    ]
    
    await update.message.reply_text(
        f"💰 Баланс: {balance:.0f} PAK ({balance/RUB_TO_PAK:.2f} ₽)\n\n"
        f"💳 **Пополнение за рубли**\n"
        f"💰 1 ₽ = 10 PAK\n\n"
        f"Выбери сумму:\n"
        f"• 50 ₽ → +500 PAK\n"
        f"• 100 ₽ → +1000 PAK\n"
        ...
        f"• 500 ₽ → +5000 PAK\n"
        f"• 1000 ₽ → +10000 PAK\n\n"
        f"💳 Оплата через ЮKassa (банковская карта)",
        reply_markup=InlineKeyboardMarkup(keyboard),
        parse_mode='Markdown'
    )
...
async def withdraw_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    balances = load_json(BALANCE_FILE)
    balance = balances.get(user_id, 0)
    rub_balance = balance / RUB_TO_PAK
    ...
    await update.message.reply_text(
        ...
        f"🪙 {balance:.0f} PAK\n"
        f"💵 {rub_balance:.2f} ₽\n\n"
        f"⏳ **Функция вывода скоро появится!**\n\n"
        f"Следите за обновлениями бота.\n"
        f"Минимальная сумма вывода: 500 ₽ (5000 PAK)\n\n"
        f"🔜 Скоро вы сможете выводить на:\n"
        f"• Карты Visa/Mastercard\n"
        f"• ЮMoney\n"
        f"• USDT (TRC20)\n\n"
...
async def play_multiplier_game(update, bet, chosen_number):
    ...
    for i, m in enumerate(multipliers, 1):
        if i == chosen_number:
            numbers_display.append(f"🎯 **{i}** → x{m}")
        else:
            numbers_display.append(f"{i} → x{m}")
...
# ========== ИГРА 3: РУЛЕТКА ==========
async def play_roulette_game(update, bet, number):
    user_id = str(update.effective_user.id)
    balances = load_json(BALANCE_FILE)
    current_balance = balances.get(user_id, 0)
    ...
    elif (number == "red" and result in [1,3,5,7,9,12,14,16,18,19,21,23,25,27,30,32,34,36]) or \
         (number == "black" and result in [2,4,6,8,10,11,13,15,17,20,22,24,26,28,29,31,33,35]):
        win_amount = bet * 2
        result_text = f"✅ ВЫИГРЫШ! x2"
    elif number == "even" and result % 2 == 0 and result != 0:
        win_amount = bet * 2
        result_text = f"✅ ВЫИГРЫШ! x2"
    elif number == "odd" and result % 2 != 0:
        win_amount = bet * 2
        result_text = f"✅ ВЫИГРЫШ! x2"
    else:
        win_amount = 0
...
async def daily_bonus(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    rub_equivalent = DAILY_BONUS / RUB_TO_PAK
    await update.message.reply_text(
        f"🎁 **Ежедневный бонус!**\n\n"
        f"🪙 +{DAILY_BONUS} PAK\n"
        f"💵 +{rub_equivalent:.2f} ₽\n"
...
async def secret_give_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("❌ /give [сумма] - выдать PAK себе")
        return
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'balance':
        ...
        await query.edit_message_text(
            "🎮 **Выбери игру:**\n\n"
            "Все игры с честной случайностью!",
        ...
        await query.edit_message_text(
            ...
            f"💰 Баланс: {balance:.0f} PAK\n\n"
            f"Напиши:\n"
            f"• чет 100 - ставка на ЧЕТ\n"
            f"• нечет 50 - ставка на НЕЧЕТ\n\n"
            f"Выигрыш: x1.8!",
        ...
        await query.edit_message_text(
            f"🎰 **Игра «Множитель»**\n\n"
            f"💰 Баланс: {balance:.0f} PAK\n\n"
            f"Напиши: множитель [сумма] [цифра]\n"
            f"Пример: множитель 100 5\n\n"
            f"Множители: 0.25x до 1.5x",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Назад", callback_data='games_menu')]])
        )
        ...
        await query.edit_message_text(
            f"🎡 **Игра «Рулетка»**\n\n"
            f"💰 Баланс: {balance:.0f} PAK\n\n"
            f"Напиши: рулетка [сумма] [число/цвет]\n"
            f"Примеры:\n"
            f"• рулетка 100 7 - на число\n"
            f"• рулетка 50 red - на красное\n"
            f"• рулетка 50 black - на черное\n"
            f"• рулетка 50 even - на четное\n"
            f"• рулетка 50 odd - на нечетное\n\n"
            f"Выигрыш: x35 на число, x2 на цвет",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Назад", callback_data='games_menu')]])
        )
        ...
        await query.edit_message_text(
            ...
            f"💰 Баланс: {balance:.0f} PAK\n\n"
            f"Напиши: кнб [сумма] [камень/ножницы/бумага]\n"
            f"Примеры:\n"
            f"• кнб 100 камень\n"
            f"• кнб 50 ножницы\n"
            f"• кнб 200 бумага\n\n"
        ...
        await query.edit_message_text(
            f"🃏 **Игра «Блэкджек»**\n\n"
            f"💰 Баланс: {balance:.0f} PAK\n\n"
            f"Напиши: блэкджек [сумма]\n"
            f"Пример: блэкджек 100\n\n"
            f"Правила: нужно набрать 21 очко\n"
            f"Выигрыш: x2!",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Назад", callback_data='games_menu')]])
        )
        ...
        await query.edit_message_text(
            f"🎲 **Игра «Кости (Сумма)»**\n\n"
            f"💰 Баланс: {balance:.0f} PAK\n\n"
            f"Напиши: кости [сумма] [число 7-12]\n"
            f"Пример: кости 100 7\n\n"
            f"Выигрыши:\n"
            f"• 7 или 12 → x5\n"
            f"• 8 или 11 → x3\n"
            f"• 9 или 10 → x2",
        ...
        await query.edit_message_text(
            ...
            f"💰 Ты получишь 10% PAK от каждого пополнения друга",
        ...
        rub = amount / RUB_TO_PAK
        await query.edit_message_text(
            f"✅ Пополнено!\n⭐ Stars → +{amount} PAK ({rub:.2f} ₽)\n💰 Новый баланс: {balances[user_id]:.0f} PAK",
            reply_markup=get_main_menu()
        )
    
    elif query.data in ['rub_50', 'rub_100', 'rub_250', 'rub_500', 'rub_1000']:
        amounts = {'rub_50': 500, 'rub_100': 1000, 'rub_250': 2500, 'rub_500': 5000, 'rub_1000': 10000}
        rub_amounts = {'rub_50': 50, 'rub_100': 100, 'rub_250': 250, 'rub_500': 500, 'rub_1000': 1000}
        amount = amounts[query.data]
        rub_amount = rub_amounts[query.data]
        balances[user_id] = balance + amount
        save_json(BALANCE_FILE, balances)
        await query.edit_message_text(
            f"✅ **Пополнено!**\n\n"
            f"💳 {rub_amount} ₽ → +{amount} PAK\n"
            f"💰 Новый баланс: {balances[user_id]:.0f} PAK\n\n"
            f"💎 1 ₽ = 10 PAK",
            reply_markup=get_main_menu(),
            parse_mode='Markdown'
...
def main():
    ...
    print("✅ Валюта: PAK (1 ₽ = 10 PAK)")
    print("✅ 7 игр доступно!")
    print("✅ Вывод средств: 'Скоро'")
...
· 1 ₽ = 10 PAK
· 1 Star = 20 PAK
· Все ставки и баланс в PAK

🎮 7 Игр:

Игра Команда Выигрыш
Кубик чет 100 x1.8
Множитель множитель 100 5 x0.25-x1.5
Рулетка рулетка 100 7 x2-x35
Слоты слоты 100 x2-x50
КНБ кнб 100 камень x2
Блэкджек блэкджек 100 x2
Кости кости 100 7 x2-x5

💳 Пополнение:

· Stars: 1 Star = 20 PAK
· Рубли: 1 ₽ = 10 PAK (через ЮKassa)

💸 Вывод средств:

· Пока показывает "Скоро"
· Минимум 500 ₽ (5000 PAK)
